...
class PerfumeBrand(models.Model):
    ...
    total_count = fields.Integer(
        string="Total Count of Perfumes", compute="_count_perfumes"
    )
    perfume_ids = fields.One2many(
        string="Current Perfume List",
        comodel_name="perfume.product",
...
class PerfumeProduct(models.Model):
    ...
    gender = fields.Selection(
        string="Gender",
        selection=[("male", "Male"), ("female", "Female"), ("unisex", "Unisex")],
    )
...
class SaleOrder(models.Model):
    ...
    accord_ids = fields.One2many(
        string="Accords", comodel_name="perfume.accords", inverse_name="sale_order_id"
    )
    accord_count = fields.Integer(
        string="Accord Count", compute="_compute_accord_count"
    )
    ...
    accord_percentages = fields.Char(
        string="Accord percentage", compute="_compute_accord_names"
    )

    @api.depends(
        "accord_ids", "accord_ids.name", "accord_ids.color", "accord_ids.percentage"
    )
    def _compute_accord_names(self):
        for record in self:
            if record.accord_ids:
                ...
                record.accord_percentages = ", ".join(
                    map(lambda perc: str(perc), record.accord_ids.mapped("percentage"))
                )
            else:
                record.accord_names = ""
